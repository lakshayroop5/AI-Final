...
class iterate {
	...
	Node* head;
	double gamma = 0.99, theta = 0.00005;
...
public:
	...
	pair<int, double> dfs(Node* temp, int currState) {
		...
		if(randoms[itr++ % 10000000] <= prob[currState]) {
			nextStateReward = dfs(temp->next, currState + 1), answerReward =  (gamma * nextStateReward.second + temp->val), maxStateReached = max(maxStateReached, nextStateReward.first), ValueFunction[currState] = (ValueFunction[currState] * (TimesVisited[currState] - 1) + answerReward)/ (TimesVisited[currState]);
