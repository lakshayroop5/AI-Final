...
public:
	...
	pair<int, double> dfs(Node* temp, int currState) {
		...
		if(randoms[itr++ % 10000000] <= prob[currState]) {
			...
			// nextStateReward = dfs(temp->next, currState + 1), answerReward = temp -> val, maxStateReached = max(maxStateReached, nextStateReward.first), ValueFunction[currState] = ((ValueFunction[currState] * (TimesVisited[currState] - 1)) + answerReward)/ (TimesVisited[currState]);
			nextStateReward = dfs(temp->next, currState + 1), answerReward =  ((gamma * prob[currState] * nextStateReward.second) + temp->val), maxStateReached = max(maxStateReached, nextStateReward.first), ValueFunction[currState] = ((ValueFunction[currState] * (TimesVisited[currState] - 1)) + answerReward)/ (TimesVisited[currState]);
			if(abs(oldValue - ValueFunction[currState])	< theta)
				stopper = true;
		}
		
		return {maxStateReached, max(answerReward, quitReward)};
	}
